function [series] = ms_ScanDCMfolder(P)
...
if ~isempty(tmp)
    %% check for "series"
    for ix=1:size(tmp,1)
        loc=strfind(tmp(ix,:),'_');
        seriesStr{ix}=tmp(ix,loc(1)+1:loc(2)-1); % series number as string
    end
...
%     seriesName=tmp(1,1:loc(3));
    ...
    for ix=1:length(allSeries)
        ...
        series(ix).DCMs = cellstr(ls(['*_' series(ix).seriesStr '_*.dcm']));
